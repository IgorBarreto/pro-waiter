    ...
    def slug_save(self):
        if not self.slug:
            while True:
                ...
                slug_exists = type(self).objects.filter(slug=self.slug)
                if not slug_exists:
                    self.slug = slug
                    break
...
class Comanda(models.Model):
    OPCOES_STATUS = (("aberta", "Aberta"), ("fechada", "Fechada"))
    ...
    status = models.CharField(
        max_length=7, choices=OPCOES_STATUS, default="aberta"
    )
    ...
    def slug_save(self):
        if not self.codigo:
            while True:
                slug = get_random_string(8)
                slug_exists = type(self).objects.filter(codigo=slug)
...
class ItemPedido(models.Model):
    ...
    status = models.CharField(
        max_length=10, choices=OPCOES_STATUS, default="preparo"
    )
